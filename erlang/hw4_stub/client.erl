...
do_join(State, Ref, ChatName) ->
	%check if client is already in chat room
...
do_leave(State, Ref, ChatName) ->
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		true -> whereis(server)!{self(), Ref, leave, ChatName},
			receive
				{_, Ref, ack_leave} -> NewState = State#cl_st{con_ch = maps:remove(ChatName, State#cl_st.con_ch)}
			end,
...
do_new_nick(State, Ref, NewNick) ->
	case NewNick == State#cl_st.nick of
		...
		false -> whereis(server)!{self(), Ref, nick, NewNick},
			receive
				{_, Ref, err_nick_used} -> whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref, err_nick_used};
				{_, Ref, ok_nick} -> whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref, ok_nick}
			end,
...
do_msg_send(State, Ref, ChatName, Message) ->
	...
	receive
		{_, Ref, ack_msg} -> whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref, {msg_sent, State#cl_st.nick}}
	end,
    {ok_msg_received, State}.
...
do_quit(State, Ref) ->
	...
	receive
		{_, Ref, ack_quit} ->  whereis(list_to_atom(State#cl_st.gui))!{self(), Ref, ack_quit}
	end,
    {ok_msg_received, State}.