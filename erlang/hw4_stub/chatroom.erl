...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	%send message to each client in chat room except sending client
	ClientList = lists:delete(ClientPID, maps:keys(State#chat_st.registrations)),
	ClientNick = maps:get(ClientPID, State#chat_st.registrations),
	lists:foreach(fun(C) -> C!{request, self(), Ref, {incoming_msg, ClientNick, State#chat_st.name, Message}} end, ClientList),
